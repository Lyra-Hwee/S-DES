...
由于我们是将每个ASCII字符看成8位，但实际上是7位，在进行加解密的时候很可能最高位变成1，导致出现一些空符号和乱码
...
对于破解一个10bit位的密钥，如果有n个明密文对只需要进行1024*n次比较即可，中途还可能出现给出的明密文对不属于同一个密钥生成的情况，我们经过一些测试后发现这种暴力破解的方式花费时间很少，多线程方式没有什么明显的改善，单线程即可
...
输入明密文对10101100，11110001

得到如下结果

![图片19](https://github.com/user-attachments/assets/2fb6a564-86c9-4b4d-8a71-f6c808b6ed9b)

随后再增加11001001，10001010

得到如下结果

![图片20](https://github.com/user-attachments/assets/88b6d7fa-bed3-4cb6-a009-6f20c449eae9)

可以很明显看到找到的密钥变少了

通过测试可以知道，随着明密文对增多，能找到10bit密钥和8bit密钥随之减少。说明对于随机选择的一个明密文对，不止一个密钥Key；同时也说明了对应明文空间任意给定的明文分组P_{n}，的确会出现选择不同的密钥K_{i}\ne K_{j}加密得到相同密文C_n的情况。存在这种冲突的现象（碰撞），是因为本身bit位的限制和其P盒、S盒进行的轮次较少，这种情况应被视为一个弱点或漏洞，现代加密算法都会努力避免这种情况的发生。
